...
def dump_predictions(pred, lbl_set, img, prefix):
    ...
    mask = np.float64(pred_lbl.sum(2) > 0)[:,:,None]
    alpha = 0.5
    img_with_label = mask * (np.float64(img) * alpha + \
            np.float64(pred_lbl) * (1-alpha)) + (1-mask) * np.float64(img)
    ...
    img_with_heatmap1 =  np.float64(img) * 0.5 + np.float64(pred_soft) * 0.5
